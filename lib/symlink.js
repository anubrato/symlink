...
var analyze = function (deps, absPaths, globals, names) {
  ...
  sorted.forEach(function (n) {
    ...
    cmds.push(cd + 'call rimraf node_modules');
    ...
    if (linked.length > 0) {
      cmds.push(cd + 'call npm link ' + linked.join(' '));
    }

    // npm install remaining deps
    var remaining = foreignDeps[n].filter($.notElem(linked));
    if (remaining.length > 0) {
      cmds.push(cd + 'call npm install ' + remaining.join(' '));
    }

    // npm link (to make this available to the modules with more dependencies)
    cmds.push(cd + 'call npm link');
  });
  return cmds;
};


module.exports = function (dirs, globals, prefixes, cb) {
  ...
  getJsonsFromDirectories(dirs, function (err, datas) {
    ...
    var names = datas.reduce(function (acc, data) {
      var namesCurr = data.filter(function (o) {
        ...
        var json = o.data;
        var name = json.name;
        // Handle peerDependencies too! Should be a cmd arg
        var mDeps = _.extend({}, json.dependencies || {}, json.devDependencies || {}, json.peerDependencies || {});
        deps[name] = Object.keys(mDeps).map(function(key) {
          return _.some(prefixes, function(pfx) { return key.startsWith(pfx) }) ? key : key + '@' + mDeps[key];
        });
